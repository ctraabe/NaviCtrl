#include "tr_serial_protocol.h"

#include "logging.h"
#include "main.h"
#include "union_types.h"


// =============================================================================
// Private data:

#define IR_START ('T')
#define SONAR_START ('S')

uint16_t ir_ = 0, sonar_ = 0;
uint8_t crc_ = 0;

static const uint8_t crc_table[] = {
  };


// =============================================================================
// Private function declarations:

static uint8_t CRC8(uint8_t * array, uint8_t len);
static void DecodeRx(uint8_t crc, uint8_t * data_buffer);


// =============================================================================
// Accessors:

uint16_t TRIR(void)
{
  return ir_;
}

// -----------------------------------------------------------------------------
uint16_t TRSonar(void)
{
  return sonar_;
}

// -----------------------------------------------------------------------------
uint16_t TRCRC(void)
{
  return crc_;
}


// =============================================================================
// Public functions:

enum UART2RxMode TRSerialRx(uint8_t byte, uint8_t * data_buffer)
{
  static size_t rx_buffer_head = 0;

  switch (rx_buffer_head)
  {
    case 3:  // Sync char
      if (byte != SONAR_START)
        goto RESET;
      else
        data_buffer[rx_buffer_head++] = byte;
      break;
    case 0:  // Sync char
      if (byte != IR_START) break;
    case 1:
    case 2:
    case 4:
    case 5:
      data_buffer[rx_buffer_head++] = byte;
      break;
    case 6:
    {
      uint8_t crc = CRC8(data_buffer, 6);
      if (crc == byte) DecodeRx(crc, data_buffer);
    }
    default:
      goto RESET;
      break;
  }

  return UART2_RX_MODE_TR_ONGOING;

  RESET:
  rx_buffer_head = 0;
  return UART2_RX_MODE_IDLE;
}


// =============================================================================
// Private functions:

static uint8_t CRC8(uint8_t * array, uint8_t len)
{
  uint16_t i;
  uint16_t crc = 0x0;

  while (len--)
  {
    i = (crc ^ *array++) & 0xFF;
    crc = (crc_table[i] ^ (crc << 8)) & 0xFF;
  }
  return crc & 0xFF;
}

// -----------------------------------------------------------------------------
static void DecodeRx(uint8_t crc, uint8_t * data_buffer)
{
  union U16Bytes temp;
  temp.bytes[0] = data_buffer[2];
  temp.bytes[1] = data_buffer[1];
  ir_ = temp.u16;
  temp.bytes[0] = data_buffer[5];
  temp.bytes[1] = data_buffer[4];
  sonar_ = temp.u16;
  crc_ = crc;

  SetNewDataCallback(LogTRData);
}
