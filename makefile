...
clean: $(BUILD_PATH)/$(LIBRARY_SRC)
	rm -f $(HEX) $(ELF) $(OBJECTS) $(DEPENDS) $(ASSEMBL)
	rmdir $(BUILD_PATH)/$(LIBRARY)/src
	rmdir $(BUILD_PATH)/$(LIBRARY)
	rmdir $(BUILD_PATH)

$(BUILD_PATH)/$(LIBRARY_SRC):
	mkdir -p $(BUILD_PATH)/$(LIBRARY)/src
